...
public class IOManager {
	...
	private boolean VerifyDatabase() {
		if (!(dbType.equalsIgnoreCase("flatfile") || dbType.equalsIgnoreCase("mysql") || dbType.equalsIgnoreCase("yml") || dbType.equalsIgnoreCase("sqlite") || dbType.equalsIgnoreCase("h2") || dbType.equalsIgnoreCase("postgre"))) {
			...
			if ((ST == StorageType.FLATFILE) || (ST == StorageType.YML) || (ST == StorageType.SQLITE)) {
				mfs = MFSConnector.getMFS(Main.pm, ST);
			} else {
				mfs = MFSConnector.getMFS(Main.pm, Main.yamlConfig.getString("Database.MySQL.Host"), Main.yamlConfig.getString("Database.MySQL.Username"), Main.yamlConfig.getString("Database.MySQL.Password"), ST);
			}
			return true;
		}
	}
	...
	private void PullDataFromDatabase() {
		...
		if (r.totalRecord() > 0) {
			for (i = 0; i < r.totalRecord(); i++) {
				...
				livePlotList.add(new Plot(ID, minPos, maxPos, plotTown, PlotType.valueOf(plotType), owner, members));
			}
		}
		//Pull chunks
		...
		if (r.totalRecord() > 0) {
			for (i = 0; i < r.totalRecord(); i++) {
				...
				liveChunkList.add(new TownChunk(Main.server.getWorlds().get(0).getChunkAt(Integer.parseInt(rec.getData(new Field("X"))),Integer.parseInt(rec.getData(new Field("Z")))), pList, rec.getData(new Field("Country")), rec.getData(new Field("Town"))));
			}
		}
		//Pull countries
		...
		if (r.totalRecord() > 0) {
			for (i = 0; i < r.totalRecord(); i++) {
				liveCountryList.add(new Country(r.getRecord(i).getData(new Field("ID"))));
			}
		}
		//Pull towns
		r = townTB.getRecords();
		if (r.totalRecord() > 0) {
			for (i = 0; i < r.totalRecord(); i++) {
				...
				liveTownList.put(newTown.getID(), newTown);
			}
		}
	...
	public void AddCitizen(String player, Town town) {
		...
		try {
			...
			mapNumber = Main.server.createMap(town.getWorld()).getId();
			setPlayerMap(player, mapNumber);
		}
