...
import me.sirsavary.townmanager.listeners.BlockListener;
import me.sirsavary.townmanager.listeners.PVPListener;
import me.sirsavary.townmanager.listeners.TownChangeListener;
...
public class Main extends JavaPlugin {
	...
	// Name of the plugin, used for tagging (messages and logging) and
	// permissions
	...
	// The first and second point maps, used to store the locations of the
	// blocks the player selected with his/her wand
	...
	// tagColor and messageColor are both used with Chatter.java to make
	// messaging the player easier
	...
	String dbType;
	// firstRun is used to determine whether or not the plugin should be
	// disabled
	...
	public void onEnable() {
		...
		try {
			...
			try {
				...
				Log
						.severe("Failed to create configuration files! Printing stack trace!");
				e1.printStackTrace();
			}
		} catch (IOException e) {
			Log
					.severe("Failed to load configuration files! Printing stack trace!");
			e.printStackTrace();
	...
	private void RegisterEvents() {
		...
		pm.registerEvents(new TownChangeListener(), this);
	}
